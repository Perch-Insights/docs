...
- `playbook-edit` clicks **Edit** on the fixture playbook, which creates a new analysis titled `<date> • Playbook Edit`, frames its header, and deletes that analysis again once the PNG is saved (or as soon as a later wait fails). If one is ever left behind, `--flow _probe find "<date> • Playbook Edit"` prints its id and `--flow _probe delete <id>` removes it.
- `pin-insights-unpinned` clicks **Unpin insight** on the fixture analysis's pinned insight, frames the unpinned state, and pins the same insight again once the PNG is saved (or as soon as a later wait fails). If it finds no pinned insight at all, it first pins the first insight, as `_fixtures` does. Check with `npm run capture -- pin-insights/click-pinned`, which fails when nothing is pinned.
- `_probe`: debugging aid. `thread <url>` or `page <url>` prints the page text and saves `capture/_probe.png`; `find <title>` prints the id and URL of the analysis with that exact title; `delete <analysis-id>` deletes one analysis whose title starts with `Docs:` or ends with `• Playbook Edit`.
